...
class Dish(models.Model):
    ...
    time_to_prepare = models.IntegerField(
        verbose_name="Czas przygotowania ( w min.)", null=True, blank=True
    )
    creation_date = models.DateField(auto_now_add=True, verbose_name="Data dodania")
    last_modified_date = models.DateField(
        auto_now=True, db_index=True, verbose_name="Data aktualizacji"
    )
...
class Menu(models.Model):
    ...
    name = models.CharField(
        max_length=128, verbose_name="Nazwa", db_index=True, unique=True
    )
